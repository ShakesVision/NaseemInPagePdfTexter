    ...
    fillText(text, ctm) {
      ...
      text.walk({
        ...
        showGlyph(font, trm, glyph, unicode, wmode, bidi) {
          ...
          const fontSize = toFixedNumber(Math.abs((trm?.[3] ?? 0) * scaleY) || Math.abs((trm?.[0] ?? 0) * scaleX));
...
function getLineTolerance(events) {
  ...
  const median = fontSizes.length ? fontSizes[Math.floor(fontSizes.length / 2)] : 18;
...
export async function openMuPdfDocument(arrayBuffer, fileName = "document.pdf") {
